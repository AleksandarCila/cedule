...
// Components
import { Box, Typography, Fab, useTheme, useMediaQuery, SpeedDial, SpeedDialAction } from "@mui/material"
...
const actions = [
    { icon: <EventIcon />, name: 'Event', modal: "NEW_EVENT", props: { tabId: 0 } },
    { icon: <AssignmentIcon />, name: 'Task', modal: "NEW_EVENT", props: { tabId: 1 } },
    { icon: <NotificationsActiveIcon />, name: 'Reminder', modal: "NEW_EVENT", props: { tabId: 2 } },
    ...
    return (
        <Box sx={{ height: "100%", display: 'flex', justifyContent: 'space-between', flexDirection: 'column', overflowY: "auto" }}>
            <Box sx={{ display: "flex", justifyContent: 'space-between', alignItems: 'center', p: 1 }}>
                <CalendarNavigationPane isMobile={isMobile} handleAddTimeClick={handleAddTimeClick} />
            ...
            >
                {actions.map((action) => (
                    <SpeedDialAction
                        ...
                        onClick={() => { handleOpenModal(action.modal, action.props) }}
                    />
                ))}
            </SpeedDial>
