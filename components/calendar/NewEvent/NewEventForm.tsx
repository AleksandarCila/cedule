...
const NewEventForm = (props: INewEventForm) => {
  ...
  const [formState, setFormState] = useState({
    ...
    timeValueStart: event
      ? event.eventStartTime
      : time
      ? new Date().setTime(time)
      : 0,
    timeValueEnd: event
      ? event.eventStartTime + event.eventLength
      : time
      ? new Date().setTime(time + 1)
      : 1,
  ...
  useEffect(() => {
    setFormValid(
      formState.nameValid &&
        formState.dateValid &&
        formState.timeStartValid &&
        formState.timeEndValid
    );
  }, [
    formState.nameValid,
    formState.dateValid,
    ...
    switch (fieldName) {
      ...
      case "allDay":
        if (!formState.allDay) {
          ...
          if (formState.timeValueEnd < 0) timeEndValid = false;
      ...
      case "timeValueStart":
        ...
        fieldValidationErrors.timeValueStart = timeStartValid
          ? ""
          : " is invalid";
        break;
      case "timeValueEnd":
    ...
    <LocalizationProvider dateAdapter={AdapterDateFns}>
      ...
      >
        <Box sx={{ width: "100%", mb: 2 }}>
          <SectionDivider title="Event Name">
        ...
        <SectionDivider title="Event Time & Date">
        ...
        <Box sx={{ width: "100%", mb: 2 }}>
          <SectionDivider title="Event Description">
        ...
        <DialogActions>
          <Button
            onClick={async () => await handleAddEvent()}
            size="large"
            variant="contained"
            disabled={!formValid}
          >
